...
public class Player extends AnimatedModel implements Cloneable {
    ...
    public Player(GLTF gltf, Vector3f position, float rotX, float rotY, float rotZ, float scale, TexturedModel shodowModel) {
        super(gltf);
        ...
        shadowNode = new Entity(shodowModel, position, rotX, rotX, rotX, 1.0f);
...
//    }
    ...
    public void jump() {
        if (!isInAir) {
            this.upwardsSpeed = JUMP_POWER;
            isInAir = true;
            setAniClipIndex(0); // Dont want to model to look like its running in the air. Would have applied a jumping animation here.
        }
    }

    private void rotateAnimatedPlayer() {
        float turnSpeed = currentTurnSpeed * DisplayManager.getFrameTimeSeconds();
